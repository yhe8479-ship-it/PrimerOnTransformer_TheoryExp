...
def main():
    ...
    rows=read_tsv(PROJECT_ROOT/'data/summarization/summary.tsv'); src_texts=[r['article'] for r in rows]; tgt_texts=[r['summary'] for r in rows]
    ...
    model=SummarizationModel(len(st),len(tt),d_model=64,pad_id=st.pad_id); opt=torch.optim.Adam(model.parameters(),lr=args.lr)
    ...
    out=PROJECT_ROOT/'run/summarization'; st.save(out/'src_tokenizer.json'); tt.save(out/'tgt_tokenizer.json'); save_checkpoint(out/'model.pt',model); print('saved to',out)
